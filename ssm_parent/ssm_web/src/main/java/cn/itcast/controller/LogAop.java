...
@Component*/
public class LogAop {
    /*@Pointcut("execution(* cn.itcast.service.impl.AopTest.*(..))")
    public void aopExpression(){}

    @Before("aopExpression()")
    public void doBefore(JoinPoint jp){
        String methodName = jp.getSignature().getName();
        String className = jp.getSignature().getDeclaringTypeName();
        String simpleName = jp.getSignature().getDeclaringType().getSimpleName();
        System.out.println("方法名:"+methodName+",全类名:"+className+",简单类名:"+simpleName);
    }*/
...
// 可以通过securityContext获取，也可以从request.getSession中获取
        ...
        sysLog.setMethod("[类名]" + jp.getSignature().getDeclaringTypeName() + "[方法名]" +
